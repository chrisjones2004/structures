package trussoptimizater.Truss.JMatrix;
...
class TestJMatrix{
    public static void main(String[] args){
        double[][] matrixArray =    {{1,2,3,4},
                                     ...
                                     {9,10,11,12},
                                     {13,14,15,16}};
        JMatrix localMatrix = new JMatrix(matrixArray);
        JMatrix globalMatrix = new JMatrix(6,6);
        int[] x2 = {0,1,4,5};
        int[] y2 = {0,1,4,5};
        globalMatrix = globalMatrix.plusEquals(localMatrix, x2, y2);
        globalMatrix.print(10, 0);
        
        int[] r = {0,1,4,5};
        globalMatrix = globalMatrix.getMatrix(r, r);
        globalMatrix.print(10, 0);
 ...
 * ------------------------ */
   ...
   public static int PRINT_WIDTH = 10;
   public static int DECIMIAL_PLACES = 3;
 ...
 * ------------------------ */
   ...
   */
   ...
   /**
    * Used when matrix A is larger than matrix B. E.g.
    * 
    * A = 3 3 3
    *     3 3 3
    * 
    ...
    * 
    * @param B      another matrix
    * @param x      Array of column indices  
    * @param y      Array of row indices.
    * @return       Asub + B
    */
   public JMatrix plusEquals (JMatrix B,int[] x,int[] y) {
      if (B.m != (x.length) || B.n != (y.length)) {
         throw new IllegalArgumentException("Matrix dimensions must agree.");
      }
      for (int i = 0; i < B.m; i++) {
         for (int j = 0; j < B.n; j++) {
            A[x[i]][y[j]] = A[x[i]][y[j]] + B.A[i][j];
         }
      }
      return this;
   }
